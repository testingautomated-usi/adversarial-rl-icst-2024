...
class Road(object):
    ...
    def step(self, dt: float) -> None:
        ...
        :param dt: timestep [s]
        """
        #print("road self vehicles: ", self.vehicles)
        for vehicle in self.vehicles:
            #print('vehicle  in self.vehicles id: ', vehicle.id)
            #print('before vehicle impact: ', vehicle.impact)
            vehicle.step(dt)
            #print('road crashed: ', vehicle.crashed)
        for i, vehicle in enumerate(self.vehicles):
            #print('vehicle in enumerate id: ', vehicle.id)
            #print('self vehicles [i+1:]' ,self.vehicles[i+1:])
            for other in self.vehicles[i+1:]:
                
                #print('other handle collisions in vehicles')
                vehicle.handle_collisions(other, dt)
            for other in self.objects:
                #print('other handle collisions in objects')
                vehicle.handle_collisions(other, dt)
            #print('after vehicle impact: ', vehicle.impact)
