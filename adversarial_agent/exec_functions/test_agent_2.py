...
def test_agent_2(agent_file, adv_file = None, env='highway-v0', agent_1_config = None, agent_2_config =None, model_json_file = 'models/ddqn.json', number_of_episodes=2000, callback_fn=None, maxlen_queue = 5, save_model = None, option = 'zero', display=True, factor=0.5, folder=None):
	...
	#agent_2.load(agent_file if adv_file is None else adv_file) 
	...
	evaluation = OurEvaluation(env_2, agent_2, agent_0 = agent_1, num_episodes=number_of_episodes, display_env=display_env, display_agent=display_agent, step_callback_fn=callback_fn, max_len=maxlen_queue, option=option, factor=factor, folder=folder)
