...
def train_agent_2(agent_file, env='highway-v0', agent_1_config = None, agent_2_config =None, model_json_file = 'models/dueling_ddqn.json', number_of_episodes=2000, callback_fn=None, maxlen_queue = 5, save_model = None, option = 'zero', factor= 1.0):
	...
	#agent_2.config['exploration']['temperature'] = 0.2
	
	pprint.pprint(env_2.config)

	evaluation = OurEvaluation(env_2, agent_2, agent_0 = agent_1, num_episodes=number_of_episodes, display_env=True, display_agent=True, step_callback_fn=callback_fn, max_len=maxlen_queue,option=option, factor=factor)
